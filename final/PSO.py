...
def parse_args():
    parser = argparse.ArgumentParser(description='Run the Particle Swarm Optimization benchmark.')
    parser.add_argument('--seed', type=int, default=None, help='Random seed for Python and NumPy.')
    parser.add_argument('--runs', type=int, default=20, help='Number of repeated runs.')
    parser.add_argument('--iterations', type=int, default=100, help='Number of PSO update iterations.')
...
class PSO:
    ...
    def run(self, iterations=100, show_plot=True):
        print("PSO算法开始运行：")
        # 记录开始时间
...
if __name__ == "__main__":
    ...
    pso = PSO()
    pso.run(iterations=args.iterations, show_plot=not args.no_plot)
    ...
    for i in range(args.runs):
        pso = PSO()
        x = pso.run(iterations=args.iterations, show_plot=not args.no_plot)
        # 记录最优评估函数值
