...
This repository provides clean, testable Python implementations of three
...
| Simulated Annealing | `metaheuristics/algorithms/sa.py` | Geometric cooling with adaptive reheating |
...
```bash
python scripts/run.py sa  --seed 42 --runs 1 --samples-per-temperature 200 --no-plot
python scripts/run.py pso --seed 42 --runs 1 --iterations 20 --no-plot
python scripts/run.py ga  --seed 42 --runs 1 --max-iteration 50  --no-plot
...
本仓库提供三种经典元启发式算法的简洁、可测试 Python 实现，
...
| 模拟退火 | `metaheuristics/algorithms/sa.py` | 几何降温，自适应再升温 |
...
適用された 3 つの古典的メタヒューリスティックアルゴリズムの
...
| シミュレーテッド・アニーリング | `metaheuristics/algorithms/sa.py` | 幾何学的冷却と適応的再加熱 |
